...
export default function CreatePost(props) {
  ...
  if(acc_type == 'artist') {
    ...
    useEffect(() => {
      const fetchAudienceId = async () => {
        try {
          const u_id = `http://127.0.0.1:8000/audience/audience-list/${username}`;
          const u_id_fetch = await fetch(u_id);
          const u_id_json = await u_id_fetch.json();
          setAudienceId(u_id_json.id);
        } catch (error) {
          console.error("Error fetching audience ID:", error);
        }
      };
  ...
  const handlePostSubmit = async (e) => {
    ...
    if(acc_type == 'artist'){
      ...
      try {
        const response = await fetch('http://127.0.0.1:8000/audience/posts/create/', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
          body: JSON.stringify({ ...data, posted_by: audienceId }),
        });
